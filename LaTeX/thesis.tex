...
\subsection{Taxonomy}
There are many use cases for using PCG in different kinds of problems with different methods and approaches. This variety of PCG possibilities made it necessary to find distinctive features and create a taxonomy of PCG to highlight the differences and similarities between approaches \cite{pcg::book}. In general, there are two different views for a taxonomy which were described by \cite{pcg::survey} and \cite{pcg::book}. The first and initial one was created by \cite{pcg::survey} and a new approach to this taxonomy was given by \cite{pcg::book} which alters and extends the class approach of \cite{pcg::survey}.\\
...
\begin{itemize}
	\item \textbf{Game Bit}: Basic elements of a game that do not affect the players gameplay. For example, procedural generated textures, sounds, vegetation like trees, fire, stone, mountains or clouds. \cite{pcg::survey}
	\item \textbf{Game Space}: Represents every environment and usually consists of different game bits. One can think of e.g. dungeons maps, whole planets with procedural generated terrain, lakes, rivers and many more. \cite{pcg::survey}
...
\end{itemize}
Whereas \cite{pcg::book} extended their view of possibilites for PCG and came up with the following seven classes:
\begin{itemize}
...
\end{itemize}
Both of the approaches are offering a good point of view for procedural generated content which can be useful when designing different game elements with PCG.
