...
This chapter is dedicated to the development of a game prototype which uses the PCGML game mechanic chosen in the last chapter. Every section in this chapter is in a direct relation with this PCGML game mechanic which does not mean that the process is not applicable for other PCGML game mechanics. Most of the described process is transferable and follows a basic principle suitable for every other kind of PCGML game mechanic. \\
The next sections contain every conceivable necessary topic to implement the PCGML game mechanic "\nameref{idea::changingWeapons}" in a game prototype. From an initial game idea to the most useful game engine to the training model and the working prototype. Note that the game idea just functions as an idea of which a prototype with an PCGML game mechanic emerges rather than being a fully sophisticated game. This chapter functions as a proof-of-concept and shows how a PCGML game mechanic can be implemented and used in a game.

\section{Test Scenario}
A two player \ac{PvP} arena-like game scenario in which each player is in control of a PCGML system. The system produces weapons such as pistols, machine guns and other kind of guns to interact. The system can be based on a specific dataset of weapons which lead the kind of game and its flow. The game forces players consistently to make use of their weapons to win the game.

\subsection{Environment and Objective}
The environment is a simple mountain-forest-like low poly environment. Players start in the same map but different locations and need to find each other to start the battle of the players which represents the the last fight of the scenario. Figure \ref{prototype::exampleMap} shows an example of the map with each player at a different spawn location and the arena in the middle.
\begin{figure}[!htbp]
	\centering
...
But first of all, they need to fight against NPC enemies on their way to the arena to specialize and get used to their weapons. But even though this is a training session, the players have a specific amount of lives and need to take care of them. If they lose all of their lives during this session then the other player wins the game and it ends.

\subsubsection{Battle of the Players}
The battle is initiated as soon as the players are in a certain range to each other. This means, that they do not particularly need to meet in the arena itself. They can also meet each other in and during the training session. As soon as they initiated the battle, the training session ends and all of their lives are restored for the final battle. Each player can now move everywhere in the map and needs to defeat the other player with his weapons to win the game. Ammunition, health and armor packages are randomly, but in player range, spawned during the battle, so that players need to walk around in the map to make it more dynamically.

\subsection{Weapon and Ammunition}
Players are able to hold two generated weapons at a time but just can use on at a time. Generated weapons come with a specific ammunition pack at generation time with the possibility to refill ammunition during the game. Players can either use a weapon as long as ammunition is available or dismantle them at any time to generate a new weapon.\\
Every generated weapon is feed into the existing PCGML model to specialize the system's model in a specific direction. Because every player is in possession of an own PCGML system, the system's are trained differently. For that reason, every system emerges in a different direction and different weapons for each player. 

\subsection{Player and Enemies}
The player is a simple first person character with specific amount of health, armor and lives with no customization options. Synchronization of the values of these mentioned properties with the properties of the generated weapons is necessary.\\
A simple AI controls all enemies to act as a training enemy for the battle. Enemies come in three different types with ascending power and health. The spawn in fixed locations throughout the training path. Enemies do not possess a complex desire, they just want to eliminate the player in a straight forward way. Killed enemies will drop ammunition, health and armor which players can gather to refill their ammunition, health and armor.

\section{Data Acquisition}
-> Available data on the web and existing games\\
it shows that games mostly use real world data for the game. observation of weapon data in Unreal Tournament and UE4 sample projects show the following used parameters:
==== insert table ====
This parameters can be used for the system and based on this data, other real data of weapons can be searched and found to get a dataset to train the model\\
\\
-> Where to find data? What is usually found?\\
could use weapon stats from CSGO\\
provide an example of a found data row and entry which is usable for the system
