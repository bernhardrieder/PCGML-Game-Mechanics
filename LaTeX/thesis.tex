...
The next sections contain every conceivable necessary topic to implement the PCGML game mechanic "\nameref{idea::changingWeapons}" in a game prototype. From the initial game idea to the most useful game engine to the training model and the working prototype.
...
\end{itemize}
Multiplayer FPS game in which players are in control of a PCGML generater which produces weapons -> mention mechanic design again!\\
Players can either use the weapons as long as ammunition is available or drop it and generate a new one. All generated weapons shall be feed into the existing model to specialize the system's model in a specific direction. As soon as the model is specialized in a weapon and only generates on specific weapon, the game starts its last few minutes in which the player can generate the last few kills.\\
\\
Game objective: get as many kills as possible, round-based like CSGO, capture the flag.. the usual FPS game modes\\
\\
(also implement basic AI? game should work with peer to peer connection, so that no server is needed)

\section{Data Acquisition}
-> Available data on the web and existing games\\
it shows that games mostly use real world data for the game. observation of weapon data in Unreal Tournamend and UE4 sample projects show the following used parameters:
==== insert table ====
...
Just generation of weapon parameters and use them to generate wepons. Visuals of weapons are not part of the focus -> is a stretch goal of the implementation
