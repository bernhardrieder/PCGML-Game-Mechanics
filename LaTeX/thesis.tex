...
For example, a PCGML system generating rules for a strategy game. This genre offers a good foundation for generating its internal rules and behavior. In particular, a strategy game mostly exists out of different sequences of actions which lead the game in a specific direction. All of those available actions have a different effect in the game whereas the effects can be changed with other appropriate effects causing different changes during the game. For example, the design of the strategy game can consist of more possibilities than it will be used in the game itself. Speaking of more game elements or available spare entities which can be used or not in the game. This kind of game element selection often applies in card board game where players need to choose the available cards in advance and as a matter of these changes, it changes the flow of the game. For instance, if a game provides the players with 25 elements and just 10 element are used in a game, then it provides, regarding the mathematical theorem of combinatorics, 3268760 possible ways to play a game. Of course only if each element will be introduced after a specific time and can't be reused. Furthermore, parts or elements of the game can be introduced in specific time slots which forces players to use the available options to move forward in the game and toward specific win conditions which can also be part of the generation. Moreover, specific conditions can be introduced in the game which need to considered in the generation. \\
In this case, the PCGML system would be trained on working configurations of the game, learns the different dependencies between each other and would be able to generate new and working configurations. 

%$ \dfrac{N!}{n!(N-n)!} = \dfrac{25!}{10!15!} = 3.268.760 $

\subsection{Changing Weapons} \label{idea::changingWeapons}
Game mechanics are often related with some kind of interactable object like weapons, guns, rifles or shotguns in \ac{FPS} games where other mechanics also evolve as a result. With this in mind, another idea of using PCGML as a game mechanic is its use as any kind of weapon generator. In specific, an FPS game with generated weapons where providing different weapons at the training time results in different matches. A virtually created weapon can have different properties, such as weight, blowback power, firing rate, bullet types and many more which can be used to procedurally generate new weapons. Furthermore, as weapons need ammunition, bullets and projectiles, they can also be generated by a PCGML system. Those mostly consist of a kind of impact damage, trajectory, speed and other properties. As it takes a lot of time to tweak the proper parameters for a fun to use weapon, PCGML can make use of its ML pillar to generate suitable weapons for a game. \\
...
The main mechanic in a lot of RPG games is fighting with a sword, weapon or applying magic spells to the enemy. Moreover, their weapons often possess a magic power as advantage or disadvantage. These powers and spells can also be generated by an PCGML system. It could affect the duration, spell category, continuing effect on the enemy or player, energy cost and other similar and often used properties.

\subsection{Solver Weapon} \label{idea::solverWeapon}
This idea is based on the co-creator pattern described in chapter \ref{AI-basedMechanicsDesignPattern}. It is about a generation weapon which helps the player to progress in the game towards a specific goal, such as solving puzzles with missing pieces in it. Special highlight of the game is, that the player needs to draw the missing pieces on a plane connected with the weapon which then hopefully generates the required piece. The detection of the drawing is done via ML and will be forwarded in chunks to the PCGML system. Drawings could consist of fat circles indicating corner and lines representing edges. For example, an introductory gameplay could be to find or generate the missing key for a door whereas the shape of the lock is shown to the player.\\
...
\begin{enumerate}
	...
	\item The PCGML system detects the drawings with any kind of object detection algorithm. As the system has obtained the chunks of the detection algorithm, it uses each chunk data to generate a fragment of a geometry and finally combines all of them into a specific shape.
\end{enumerate}
Furthermore, this can be extended with the trainee and guided pattern where the player would provide the system with feedback if the PCGML system outputs the wrong geometry. In this case, it would learn the specific drawing conventions of a player and adjust itself to the player.

\subsection{Defeat of the Enemy} \label{idea::defeatTheEnemy}
The PCGML system tries to defeat the player and one or more players try to defeat the enemy. This is based on the adversary design pattern described in chapter \ref{AI-basedMechanicsDesignPattern} where the role of the AI is to defeat the player. In this case, the system is trained on simple player behavior, such as walking paths in a specific environment and uses this knowledge to overcome the player. Moreover, it would be also able to find new behavior pattern during the fight. Summarized, the output of the system are new tactics, behaviors or sequences for the AI on how to defeat a player. Furthermore, the PCGML system can categorize used behavior regarding their success or failure and learn from these outcomes to increase its accuracy. \\
...
For example, a game in which a player controls a thundercloud and has the objective to destroy cities or enemies with the power of lightning. In this scenario, a PCGML system would generate pattern for the diffusion of lightning and impact points based on different weather conditions, cloud height, enemy or building position, physical materials and similar properties. Could be used as a kind of a thunderstorm simulation game which is based on a trained PCGML model. Could be based on empirical or real data.

\subsection{Train to Progress} \label{idea::trainToProgress}
A player is provided with a completely empty and untrained PCGML system. The system would be able to generate an important part for the gameplay but needs to be trained to activate the functionality of generating necessary parts. In this case, a player's objective is to train a PCGML system to progress towards a given goal. For instance, a puzzle game where the main puzzle is the system's training to generate, e.g., some missing piece. There could be hidden objects in a level which needs to be found first to properly train the system. For example, if the objective is to unlock a door with a generated key then the player needs to find sample keys first to train the system with those which are similar to the locks key. If there isn't any success in generating the right key then the player can reset the systems internal model and try it again with another pair of sample keys. This principle could be done with other kinds of objects too. This idea is mainly based on the trainee design pattern.

\subsection{Building with Assistance} \label{idea::buildingWithAssistance}
The PCGML system is trained to generate incomplete game elements for a specific goal. For example, a building game where the PCGML system never outputs the correct element which is currently needed. On the other hand, the player has the abilities to improve and overhaul the generate elements. All changes will be then fed into the system which guides its internal model to a desired generation output direction. Thus, this idea is based on the guide design pattern.

\subsection{The exploring Co-Worker} \label{idea::exploringCoWorker}
An idea based on the co-creator pattern would be, e.g., a strategy alike game where the player's focus is to mine, build and create strategic plans. Furthermore, the player possesses a PCGML system which is used to explore and conquer new sections of a map. Other than the guidance needed PCGML system, this system works completely on its own and functions as a helper for the player. Exploration of a new section can be figured by creating new land by the system. In this case, the system is trained to generate new chunks or section of unexplored terrain mainly based on the player's progress, mined resources and similar elements.

\subsection{Observe and Learn} \label{idea::observeAndLearn}
A PCGML system which is trained to generate terrains with a special path within the terrain to reach a certain goal. The highlight is that the system doesn't always generate possible ways which demands the player to change properties of the system which changes the level, map or world. In doing so, the player is forced to observe the interactive and smooth changes of the system to reach the current goal. This idea is based on the visualize and editable design pattern.

\subsection{Express Yourself} \label{idea::expressYourself}
A Point'n'Click game where a PCGML system is trained to generate new storylines, events, choices and outcomes. The system is trained on existing designed stories and learns the dependencies to create new and novel storylines which individual possible choices and outcomes. Like in the idea of \ref{idea::rulesAndBehavior}, the designer can provide more game elements than actually used in the game which provides the system which a various possibilities. This is mainly based on the spectacle pattern.

\subsection{Big Boss Helper} \label{idea::bigBossHelper}
This idea is based on the villain pattern in which the AI just mobs a player. This fits perfectly in, e.g., a big boss fight with more than one NPC to defeat. For example, there are two NPC's to defeat - one applies mainly damage and the other one protects the other one. This helper NPC is represented by the PCGML system which is trained on player behavior in a specific level or environment to generate or alter the terrain or level to keep the player off the main enemy in the fight.

\subsection{Figure it Out} \label{idea::figureItOut}
A multiplayer game where each player possesses a PCGML system for generation purposes but each generation changes the other's system a little and they need to work togheter towards a specific goal. For instance, a puzzle game where players need to figure out the connection between their system. The PCGML system would be trained with a specific game corpus and enables interaction based on the editable pattern. The player changes parameters and also changes the other while they need to generate an object combined with each of the systems output. The change of parameters could happen directly or indirectly with any kind of minigame like moving stones around in a room and find the right position. Furthermore, this idea can also be used in a single player puzzle game.

\subsection{Novel Cars} \label{idea::novelCars}
Another possible field of application for a PCGML system can be the generation of cars or vehicles as they also consist of many properties and parameters.

\section{Evaluation of Concepts}
Conceptual Prototypes (with evaluation)\\
A test implementation of PCGML game mechanics in a game engine.

The main issue in training the ML model will be the requirement of a useful and "quite big" training set of more than or around 50 different basic elements. (really? is this a valuable amount?)

\subsection{\nameref{idea::rulesAndBehavior}}
the sequence of actions, effects and game changes can be shown as a tree and easy to use. \\
The game elements can require other elements to properly work and vice versa which can be a tough finetuning process for a standard PCG algorithm\\
The PCGML system can generate based on trained sequences similiar ones which can work out for a cool game flow
\subsubsection{PRO}
\begin{itemize}
	\item could offer a broad range of how to play a game
	\item offers players to save a learned model and output for other matches and preferences
	\item with providing more designed content for training offers more possibilites
\end{itemize}
\subsubsection{CON}
\begin{itemize}
	\item needs a lot of predesigned sequences of rules, behaviors and outcomes
	\item training of ML model can be hard because there are many elements to consider and dependencies -> better to contentrate on just one specific topic
	\item predesigned sequences need to be tested in order to function
	\item needs design of a game with much spare parts which can be replaced by one another -> needs a good game design
\end{itemize}

\subsection{\nameref{idea::changingWeapons}}
\subsubsection{PRO}
\begin{itemize}
	\item can use existing open source games for implementation 
	\item not as many dependencies between values as in rules and behaviors
	\item it doesn't matter if the generated weapons work properly
\end{itemize}
\subsubsection{CON}
\begin{itemize}
	\item
...
\subsubsection{PRO}
\begin{itemize}
	\item doesn't need success
	\item spells can be anything
	\item don't consist of a complex structure
\end{itemize}
\subsubsection{CON}
\begin{itemize}
	\item needs good design of balanced and tweaked spells first
	\item visuals should also be part of the generation process
\end{itemize}

\subsection{\nameref{idea::solverWeapon}}
\subsubsection{PRO}
\begin{itemize}
	\item training model can easily provided
	\item easy to learn for a player
\end{itemize}
\subsubsection{CON}
\begin{itemize}
	\item needs two trained ML systems
	\item game design needs properly thought through
\end{itemize}

\subsection{\nameref{idea::defeatTheEnemy}}
\subsubsection{PRO}
\begin{itemize}
	\item offers new player experience thorugh new way of 
\end{itemize}
\subsubsection{CON}
\begin{itemize}
	\item model won't operate properly until a quite time because it need possible paths ->pretrained
	\item PCG can be complex
\end{itemize}

\subsection{\nameref{idea::caughtInAThunderstorm}}
ML could be trained on L system (plants, etc) which is trained on the described properties. the ml output a kind of sequence according to the prior trained sequences.
\subsubsection{PRO}
\begin{itemize}
	\item pattern are easy to implement
	\item can be implemented with statistical, empirical and real data
\end{itemize}
\subsubsection{CON}
\begin{itemize}
	\item game would need a good point why to play and is fun
	\item changes of thunderstorm -> how to obtain them?
\end{itemize}

\subsection{\nameref{idea::trainToProgress}}
\subsubsection{PRO}
\begin{itemize}
	\item doesn't need training to work properly -> is done in the game
	\item can be fun if game design is done well and model works fine
\end{itemize}
\subsubsection{CON}
\begin{itemize}
	\item takes a time to properly train the model by the player -> needs a good tutorial
	\item can be frustrating if the model doens't learn properly
\end{itemize}

\subsection{\nameref{idea::buildingWithAssistance}}
\subsubsection{PRO}
\begin{itemize}
	\item PCG is easy to implement
\end{itemize}
\subsubsection{CON}
\begin{itemize}
	\item how to secure that only usable elements will be generated?
	\item can be frustrating if nothing right is generated
	\item model takes a while to get properly guided
\end{itemize}
