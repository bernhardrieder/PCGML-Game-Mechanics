...
\begin{table}[!ht]
	...
	\begin{tabular}{|l|p{3.5cm}|p{3.5cm}|}
		\hline
		\textbf{Output Parameter}       & \textbf{Training Weapon} & \textbf{Generated Weapon} \\ \hline\hline
	...
	\end{tabular}
	\caption{Comparation of a training weapon and a generated weapon with random noise as input.}
...
tensorflow works like that the creates nodes have a fixed shape after the training. for this reason, the batch size for encoding and decoding needs to be the same as the training batch size. that is because batch size is propagates thorugh every node in the network and trained on this shape. this means someone can either input a whole batch of random numbers or existing weapons and generate the whole batch. Or, i also implemented that it is possible to just input one simple input which is replicated to the size of the batch and outputs the mean of all generated weapons.
