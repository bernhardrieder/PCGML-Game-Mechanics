...
A two-player \ac{PvP} or \ac{PvE} arena-like 3D game scenario in which each player or AI opponent is in control of a PCGML system which produces weapons such as pistols, machine guns and other kinds of guns to interact. Specific datasets of weapons can bias the system which leads the kind of game and its flow. The game forces players consistently to make use of their weapons to win the game. 

\subsection{Environment and Objective}
The environment is a mountain-forest-like environment. Players start on the same map but different locations and need to find each other to start the battle of the players which represents the last fight of the scenario. Figure \ref{prototype::exampleMap} shows an example of the map with each player at a different spawn location and the arena in the middle. The player's primary objective is to defeat the other player in a battle against each other at the end of the scenario. The winner of the battle wins the game.
\begin{figure}[!htbp]
	\centering
...
\end{figure}
The players main objective is to defeat the other player in a battle against each other at the end of the scenario. The winner of the battle wins the game. 

\subsubsection{Training Session}
However, first of all, they need to fight against NPC enemies on their way to the arena to specialize and get used to their weapons. Even though this is a training session, the players have a specific amount of lives and need to take care of them. If they lose all of their lives during this session, then the other player wins the game, and it ends.

\subsubsection{Battle of the Players}
Initialization of the battle happens as soon as the players are in a specific range to each other. This specific range means that they do not particularly need to meet in the battle arena itself. They can also meet each other in and during the training session. As soon as they initiated the battle, the training session ends and recovers every player's lives for an epic final battle. Each player can now move everywhere on the map and needs to defeat the other player with his weapons to win the game. Ammunition, health, and armor packages are randomly, but in player's range, spawned during the battle, so that players need to walk around in the map to add more energy in the battle.

\subsection{Weapon and Ammunition}
Players can hold two generated weapons at a time but can use on at a time. Generated weapons come with a specific ammunition pack at generation time with the possibility to refill ammunition during the game. Players can either use a weapon as long as ammunition is available or dismantle them at any time to generate a new weapon.\\
Every generated weapon is feed into the existing PCGML model to specialize the system’s model in a specific direction. Because every player is in possession of an own PCGML system, the system’s will evolve differently. For that reason, every system emerges in a different direction and different weapons for each player.

\subsection{Player and Enemies}
The player is a first-person character with a specific amount of health, armor, and lives with no customization options. Synchronization of the values of these mentioned properties with the properties of the generated weapons is necessary.\\
...
The UE4 TensorFlow plugin currently only supports the Windows OS. To get started with the plugin, it needs the installation of TensorFlow on the executing system. TensorFlow supports 64-bit and x86 desktops or laptops with Windows 7 or later \cite{api::tensorFlow}. There are two installation options for enabling different types of TensorFlow on the PC:
\begin{itemize}
...
The search for appropriate data starts with the sample and template projects provided in UE4 since its the engine used for the prototype. The templates can offer a great advantage because they could provide usable data and implementation for the prototype. Unfortunately, this template projects are not eligible and the search ends with the conclusion of either using real world data or a games data. The next chapters are going into the detail of all evaluated data sources.
...
It shows that Unreal Tournament does not use much weapons as needed for the training. The implementation contains only 16 different weapons with 10 common and usable parameters to train the model. Nevertheless, the source code of their weapons and used parameters are the perfect reference or template for implementing an own weapon class. \\
The other starting point for finding appropriate data is the so-called "Shooter Game" project which is provided as a introduction for implementing FPS games \cite{epicGames::shooterGame}. Unfortunately, this project does not provide much usable data. In fact, it only provides the developer with basic implementation for two different kind of weapons with an example for each kind of gun. The parameters used in their implementation are similar to Unreal Tournament's weapon parameters.

\subsubsection{\acl{CS:GO}}
The next point of an initial reference is the popular eSports game \ac{CS:GO}. Websites about community-driven analysis on the game's main mechanic is the perfect base for finding data appropriate to train a ML model and use the data for weapon generation. Unfortunately, CS:GO does not use much weapons and is specialized to 32 weapons in the game \cite{counterstrike::wikiWeapons}. Nevertheless, the provided analysis data provides a good base to work with and to extend the available and balanced game data with, e.g., real world data.

\subsubsection{Real World Data}
Plenty of websites about weapons are findable on the \ac{WWW}. However, most of them do not provide much data about the weapon itself. The search leads to, e.g., Army and manufacturer websites to find the appropriate weapon data for specific weapons. Nevertheless, the procedure in finding and obtaining real world weapon data is a lenghty process. Besides, it shows that weapon manufacturer do not always provide all data and it would take real world tests to obtain particular weapon data. In particular, all found weapon data have about 7 to 8 similar variables which are usable to train the model. This parameters are: type of weapon, average weight, overall length, barrel length, bullet caliber, magazine size, muzzle velocity, and the rate of fire. \\
For sure, this data is usable to train a model and generate weapons even if this takes more time to implement and balance. However, it would be easier to use already balanced game data to train the model and generate new weapons. For this reason, the search leads to the most valuable data source which is furthermore used for the implementation.

\subsubsection{Battlefield 1}
Research showed, that the game Battlefield 1 which was released in 2016, uses more than 100 different weapons in the game. The setting of the game takes place in the World War One and therefore uses weapons of this period. Fortunately, gamers gathered all possible weapon statistics for Battlefield games on the community website "http://symthic.com" \cite{symthic::bf1stats}. They provide a list of all 140 weapons used in the game with very detailed description and statistics \cite{symthic::bf1stats}. In fact, they provide 40 parameters which describe every weapon \cite{symthic::bf1stats}. Further evaluation specializes this parameters to 18 most usable variables for a weapon implementation. Nevertheless, one can use even more, less or other parameters to generate weapons and train the model and not all of them are always useful. Table \ref{battlefield::weaponStats} shows this most usable parameters for the weapon class and model training. As already indicated, the number of the variables is a subject to change during the implementation.
\begin{table}[!htbp]
	\centering
	\begin{tabular}{|l|l|}
		\hline
...
trying different models if your model sucks! -> use WEKA as a rescue http://www.cs.waikato.ac.nz/ml/weka

learning with logs! -> training set

santi said: either NN or Gaussian Mixture Model

As seen throughout the previous chapters, there is no distinctive right or wrong method to train a model. PCGML is relatively new and someone can just guess which one is usable. Best approach would be to use different methods and compare them. For simplicity, onle the method of NN is approached for this given problem. If there is enough time left then other methods can also be tested
