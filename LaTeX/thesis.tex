...
\subsection{Changing Powers} \label{mechId4}
...
\subsection{Solver Weapon} \label{mechId5}
...
\subsection{Defeat of the Adversary} \label{mechId6}
The PCGML system tries to defeat the player and one or more players try to defeat the enemy. This is based on the adversary design pattern described in chapter \ref{AI-basedMechanicsDesignPattern} where the role of the AI is to defeat the player. In this case, the system is trained on simple player behavior, such as walking paths in a specific environment and uses this knowledge to overcome the player. Moreover, it would be also able to find new behavior pattern during the fight. Summarized, the output of the system are new behaviors or sequences for the AI on how to defeat a player. Furthermore, the PCGML system can categorize used behavior regarding their success or failure and learn from these outcomes to increase its accuracy. \\
...
\subsection{Little Guidance Needed}
...
\subsection{Co-Worker - Exploring with PCGML}
...
\subsection{Express Yourself} 
A Point'n'Click game where a PCGML system is trained to generate new storylines, events, choices and outcomes. The system is trained on existing designed stories and learns the dependencies to create new and novel storylines which individual possible choices and outcomes. Like in the idea of \ref{mechId1}, the designer can provide more game elements than actually used in the game which provides the system which a various possibilities. This is mainly based on the spectacle pattern.

\subsection{Big Boss Helper}
...
\subsection{Novel Cars} 
Another possible field of application for a PCGML system can be the generation of cars or vehicles as they also consist of many properties and parameters. \label{mechId3}
...
\\
idea chapter \ref{mechId1}:\\
This is for the comparison! -> the sequence of actions, effects and game changes can be shown as a tree and easy to use. \\
