...
\place{Philadelphia}
\kurzfassung{
	Prozedurale Inhaltsgenerierung ist ein wichtiges Thema in modernen Videospielen, denn sie trägt eine erhebliche Hilfe bei der Erstellung von Spielinhalten bei. Neue Forschungen haben nun mit maschinellem Lernen neue Horizonte für prozedurale Inhaltsgenerierung erschlossen. Dabei wurde festgestellt, dass es noch viel herauszufinden gibt, wie etwa die Verwendung von prozeduraler Inhaltsgenerierung durch maschinelles Lernen als Spielmechanik.\\
	Daher beschäftigt sich diese Arbeit mit der Frage, wie man prozedurale Inhaltsgenerierung durch maschinelles Lernen als Spielmechanik verwenden kann, was im Folgenden nur noch als Spielmechanik bezeichnet wird. Die Arbeit folgt dem Ansatz einen Leitfaden für Entwickler zu erschaffen, dass diese mit theoretischem und praktischem Wissen für eine Entwicklung vorbereitet. Es wird zuerst theoretisches behandelt und danach mögliche Spielmechanik Konzepte vorgestellt, woraus eine der Mechaniken als Prototyp entwickelt wird. Die Entwicklung des Prototypen wurde dokumentiert, sodass Spielentwickler dieser Entwicklung bei Bedarf folgen können.\\
	Die Arbeit zeigt, dass sich die Spielmechaniken für eine große Palette von Spielgenres eignet. 13 verschiedene Konzepte von Spielmechaniken werden vorgestellt, woraus die Spielmechanik "Wechselnde Waffen" für einen Prototypen auserkoren wurde. Das Hauptmerkmal dieser Mechanik ist, dass sie einen Waffen Generator besitzt, welcher neue aber ähnliche Waffen basierend auf Waffen eines bekannten Ego-Shooters generieren kann. Der Generator verwendet einen in TensorFlow implementierten sogenannten "Variational Autoencoder" um die verborgene Struktur der zur verfügung gestellten Waffen zu lernen und ist in der Lage sinnvolle Waffen Daten zu generieren. Der Generator wurde anschließend in Unreal Engine 4 integriert um zu beweisen, dass diese Spielmechaniken in häufig verwendeten Spieleentwicklungsumgebungen ohne Probleme funktionieren und zeigt, dass es so ist.\\
	Um die Arbeit abzuschließen wurde ein Leistungs Bericht erstellt welcher zeigt das die entwickelte Spielmechanik keine großen Leistungseinbußen mit sich bringt. Das bedeutet, dass es möglich ist diese Spielmechaniken in gewöhnlichen Videospielen zu verwenden. Daher zeigt dieser Machbarkeitsnachweis, dass für zukünftliche Spiele eine neue Ära von Spielmechaniken angebrochen ist.
}
\schlagworte{Prozedurale Inhaltsgenerierung, Machine Learning, Spielemechanik, Künstliche Intelligenz, Spieleentwicklung, Neuronale Netze}
\outline{
...
\section{\acl{VAE}}
The implementation of the VAE was driven and inspired by an explanation of \citep{tutorial::vaeTF}. \citename{tutorial::vaeTF} used a high-level approach in his design which is similar to high-level ML APIs. These high-level APIs are easily modifiable and extensible which was the main reason why to follow the approach of \citename{tutorial::vaeTF}. An easy modification is necessary to test different network hyperparameters without significant changes in the code and thus to find the most suitable network parameters. In particular, chapter \ref{chapter::vae::network} explains the entire proceeding of the hyperparameters selection process. Now, the next chapters show some coder overviews of the VAE class, how the model accuracy was measured to obtain an adequately trained model, addresses the already mentioned the network parameter selection, shows two weapon generation examples and lists development issues.
...
The focus of this thesis was to provide theoretical possibilities for PCGML game mechanics and a practical development example. Chapter \ref{chapter::possibleGameMechanics} provides these possibilities and furthermore show some pros and cons of theoretical development. The game mechanic "\nameref{idea::changingWeapons}" was then implemented and introduced the development workflow of a PCGML game mechanic. Overall, the thesis fulfilled all expectations of using PCGML as a game mechanic and shows valuable research results.
