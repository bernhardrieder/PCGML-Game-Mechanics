...
The first one is utilization which is one of the main argument why PCG is popular. It can be time-saving because it could produce more content than a human in an hour, for instance a whole galaxy in No Man's Sky \cite{noMansSky}, PCG overcomes technical limitations in terms of their use for devices with e.g. limited space like mobile devices, is expandable and has reusable code due to modularity and same field of applications and it increases replayability because it can generate many but different instances of a content \cite{pcg::inGameDesign}.
...
There are many games which are using procedural generated content in their game but not all of them are using PCG as a game mechanic. Some games which are using PCG as a game mechanic are described in the following sections.
