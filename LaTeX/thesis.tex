...
\subsection{Bots}
The bots are implemented as requested and come in 3 different types. The easiest one is a simple ball bot which moves towards the player and explodes after some time after reaching the player. The two other bots are from the same type  which is visible in figure \ref{scenarioScreenshot}, moves towards the player and shoots with different weapons onto the player in a straight forward way. The easy bot does not cause much damage but tries to heal itself if the health is low whereas the heavy bot causes more damage, has more health and does not heal itself.\\
