...
\begin{comment} 
\subsection{Short \acl{ANN} Primer}
\subsubsection{Neurons and Feed Forward Networks}
Feed forward networks are the simples networks used in neural networks. there just 
\subsubsection{Backpropagation}
is how neural networks learn. they apply different optimizer to detect the delta of the weights and biases. there is for example, gradient descent or stochastic gradient descent.
\subsubsection{Workflow}
\begin{enumerate}
	\item Randomly initialize weights to small numbers close to 0
	\item input data X
	\item feed forward until predicted result y
	\item compare predicted result to real result and measure error
	\item back propagate error and update weights and biases
	\item repeat steps 2 - 5 after each batch
	\item whole trainning set passed through -> epoch complete and restart of 2 until all epochs are finished
\end{enumerate}
