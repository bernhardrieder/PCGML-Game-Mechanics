...
A development example of a generation process for continuous variables in a sequence data structure is given in this section and the detailed development for a PCGML system is given in chapter \ref{chapter::gamePrototype}. For this example, a data's structure example for a PCGML system provides figure \ref{fig::PCGML::devExample::sequenceBase}. As it is a sequence data, all values are connected to each other. In the figure, the S node represents a start value and all other nodes represent data row values. 
\begin{figure}[!htbp]
	\centering
