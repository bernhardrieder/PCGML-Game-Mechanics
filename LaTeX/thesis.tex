...
\section{Development}
There are some special development considerations to take when someone wants to make use of PCG via ML for game elements. In general, every design consideration, possible issue, pitfall and best practices described and discussed in chapters \ref{pcgDevelopment} and \ref{mlDevelopment} will apply when implementing PCGML algorithms. Furthermore, the data representation for the data used as the training set during the ML model's training needs to be examined first - this is described in the next chapter. Moreover, the research of \cite{pcgml::paper} showed that every specific dataset has different possible training methods for the ML model to obtain good and practical generation results. The primarily used training models for their research purposes of using PCGML for, e.g., level generation were:
\begin{itemize}
...
One of the essential parts in implementing a PCGML system is the data representation of the data used as the training set for the ML model's training method. One can easily see a clear difference between the prediction of game elements and standard prediction problems such as the prediction of, e.g., an employee's monthly salary. The latter problem's training set is mostly represented in a simple datastructure with some dependent values and a value which it needs to predict whereas this is not possible for a game element. In their survey focused on using PCGML systems mainly for level generation, \cite{pcgml::paper} approached this problem with different techniques and tried different training models with each data representation. In general, they used the following three different techniques which were further specialized for the given game and level to generate:
\begin{enumerate}
	\item \textbf{Sequences}: Represents a format for content which shows a natural order in its structure \cite{pcgml::paper}. For example, the Super Mario Bros. levels consist of this natural format because the player can only traverse from left-to-right and it can be represented in a sequence \cite{pcgml::paper}. Moreover, sequences apply also for textual elements \cite{pcgml::paper}.
	\item \textbf{Grids}: This type of representation apply for game content which can be thought of as a two-dimensional grid \cite{pcgml::paper}. For example, a top down view of a labyrinth is representable as a grid.
	\item \textbf{Graphs}: Are a general data representation for game content with the drawback of lack of well-defined structural properties \cite{pcgml::paper}. 
\end{enumerate}
...
how does it work? thats the real question!
