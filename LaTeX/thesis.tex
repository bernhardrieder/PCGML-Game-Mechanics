...
\begin{enumerate}
	\item \textbf{Sequences}: Represents a format for content which shows a natural order in its structure \cite{pcgml::paper}. For example, the Super Mario Bros. levels consist of this native format because the player can only traverse from left-to-right and it can be represented in a sequence \cite{pcgml::paper}. Moreover, sequences apply also to textual elements \cite{pcgml::paper}.
